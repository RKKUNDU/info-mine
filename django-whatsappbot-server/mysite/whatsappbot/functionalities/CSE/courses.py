...
class Courses:
    def show_courses(self, in_autumn = True, in_spring = True):
        ...
        try:
            ...
            return (tabulate(table, headers = ['Semester', 'Course Code', 'Course Name', 'Instructor Name']))
    ...
    def show_filtered_courses(self, filter, with_details = False, with_description = False, with_prereqs = False, with_textrefs = False, in_autumn = True, in_spring = True):
        ...
        try:
            ...
            if len(table) > 0:
                return (tabulate(table, headers = ['Semester', 'Course Code', 'Course Name', 'Instructor Name']))
            elif (with_details or with_description or with_prereqs or with_textrefs) and cnt >= 1:
                return ret
            else:
                return ("No courses found with this filter. Please try with some other filter")
